...
def create_preview_token(data: dict, expires_delta: timedelta | None = None) -> str:
    """Create a signed JWT token specifically for previews."""
    to_encode = data.copy()
    expire = datetime.now(timezone.utc) + (
        expires_delta or timedelta(minutes=30)
    )
    to_encode.update({"exp": expire, "type": "preview"})
    return jwt.encode(to_encode, settings.JWT_SECRET, algorithm=settings.JWT_ALGORITHM)

def decode_preview_token(token: str) -> dict:
    """Decode and validate a preview token. Ensures type is 'preview'."""
