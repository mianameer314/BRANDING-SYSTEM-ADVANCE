...
export function CaseStudiesPage() {
 ...
 const hasActiveFilters = !!filters.search || !!filters.status || !!filters.industry || filters.is_featured !== undefined || !!filters.sort_by;

 const handleDelete = async () => {
 if (!itemToDelete) return;
 try {
 await deleteCaseStudy(itemToDelete);
 toast.success('Case study deleted successfully');
 if (data?.items.length === 1 && page > 1) {
 setFilter('page', page - 1);
 }
 } catch (err: any) {
 toast.error(err?.response?.data?.detail ?? 'Failed to delete case study');
 } finally {
 setItemToDelete(null);
 }
 };

 const columns: ColumnDef<CaseStudyOut>[] = [
 {
 key: 'title',
 header: 'Title',
 render: (row) => (
 <div>
 <p className="font-medium text-foreground">{truncate(row.title, 50)}</p>
 ...
 { key: 'client_name', header: 'Client', width: 'w-36', render: (row) => <span className="text-foreground">{row.client_name}</span> },
 ...
 showIndustry
 showIsFeatured
 industries={CASE_STUDY_INDUSTRIES}
 />
 ...
 <EmptyState title="No case studies yet" description="Published case studies will appear here." />
 )
 )}
