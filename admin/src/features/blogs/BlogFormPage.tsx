...
export default function BlogFormPage() {
  ...
  const handleSecurePreview = async () => {
    ...
    try {
      ...
      window.open(`${env.frontendUrl}/preview/blog?token=${token}`, '_blank');
    } catch (_error) {
  ...
  const handleApplyDraft = (generated: BlogGeneratedContent) => {
    ...
    try {
      if (isEdit && existing) {
        ...
        if (shouldPreviewAfterSave.current) {
          ...
          try {
            setIsGeneratingToken(true);
            const token = await generatePreviewToken({ content_type: 'blog', content_id: existing?.id || 0 });
            window.open(`${env.frontendUrl}/preview/blog?token=${token}`, '_blank');
          } catch (_error) {
            toast.error('Failed to generate preview token');
          } finally {
        ...
        if (pendingResources.length > 0) {
          ...
          const { successCount, failCount } = await uploadPendingResources(pendingResources, 'blog', newContent.id);
          setIsUploadingResources(false);
          setPendingResources([]);
          if (failCount === 0) {
            toast.success(`Blog created successfully. ${successCount} resources uploaded.`);
          } else {
            toast.error(`Blog created. ${successCount} resources uploaded, ${failCount} failed.`);
          }
        } else {
  ...
  if (isEdit && isError) return <ErrorState message="Blog not found." />;
    ...
    <>
      <div className="mx-auto max-w-5xl space-y-6">
        <form onSubmit={handleSubmit(onSubmit)}>
          <ContentFormLayout
            title={isEdit ? 'Edit Blog' : 'New Blog Post'}
            subtitle={isEdit ? `Editing: ${existing?.title}` : 'Create a new blog post'}
            headerAction={
